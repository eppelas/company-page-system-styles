...
const ChapterHeader = ({ number, title }: { number: string; title: string }) => (
  <div className="flex justify-between items-end border-b border-white/20 pb-4 mb-12">
    ...
    <div className="font-serif italic text-xl md:text-2xl">{title}</div>
  </div>
);
...
export default function AiMindsetWireframeContentPage() {
    ...
    <div ref={containerRef} className="h-screen overflow-y-scroll bg-[#080808] text-white selection:bg-white selection:text-black hide-scrollbar font-inter relative">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 px-6 py-6 flex justify-between items-start mix-blend-difference">
        <div className="flex items-center gap-2 text-[10px] tracking-widest uppercase font-medium">
          <img src="/assets/ai-mindset-logo.png" alt="AI Mindset logo" className="h-4 w-4 object-contain" />
          <span>AIM 48 · Company Page System</span>
        </div>
        <div className="font-serif italic text-xl absolute left-1/2 -translate-x-1/2">system</div>
      ...
      <section className="min-h-screen relative flex flex-col justify-center items-center overflow-hidden">
        ...
        <div className="relative z-10 w-full px-4 md:px-12">
          <div className="flex justify-between items-end border-b border-white/20 pb-2 mb-2">
            ...
            <span className="text-[10px] uppercase tracking-widest text-white/80 font-medium">Blueprint</span>
          </div>

          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <h1 className="text-[22vw] leading-[0.8] font-medium tracking-tighter mix-blend-difference text-white/95 drop-shadow-[0_0_24px_rgba(255,255,255,0.2)]">COMPANY</h1>
            <div className="hidden md:block w-px h-32 bg-white/20 mx-8" />
            <h1 className="text-[22vw] leading-[0.8] font-medium tracking-tighter mix-blend-difference text-white/95 drop-shadow-[0_0_24px_rgba(255,255,255,0.2)] text-right md:text-left">PAGE</h1>
          </div>
          <h1 className="text-[22vw] leading-[0.8] font-medium tracking-tighter mix-blend-difference text-white/95 drop-shadow-[0_0_24px_rgba(255,255,255,0.2)] text-center md:text-right">SYSTEM</h1>
        </div>

        <div className="absolute bottom-6 left-6 text-[10px] tracking-widest">01 — 04</div>
      </section>

      <section className="min-h-screen py-24 px-6 md:px-12 relative">
        <ChapterHeader number="II" title="The page system" />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-24">
          <div className="pt-12 md:pt-24">
            <p className="text-2xl md:text-4xl leading-tight font-light text-white/85">
              Not a one-off page, but a controlled system for company sites. One visual language, many outputs,
              and a clean path from brief to launch.
            </p>
            <div className="mt-12 grid grid-cols-3 gap-3 max-w-xs">
              ...
              <div className="w-12 h-12 border border-white/30 rounded-full flex items-center justify-center text-xs">03</div>
            </div>
          </div>
      ...
      <section className="min-h-screen flex items-center justify-center bg-white text-black relative overflow-hidden">
        <div className="absolute top-6 left-6 text-[10px] uppercase tracking-widest">Chapter (III)</div>

        <div className="w-full">
          <div className="border-y border-black/10 py-12 overflow-hidden">
            <motion.div
              className="whitespace-nowrap text-[14vw] leading-none font-medium tracking-tighter flex gap-12"
              animate={{ x: ['0%', '-50%'] }}
              transition={{ duration: 20, repeat: Infinity, ease: 'linear' }}
            >
              BRIEF → BRAND → BLOCKS → LAUNCH {`{SYSTEM}`} —
            </motion.div>
          </div>
        </div>

        <div className="absolute bottom-12 right-12 w-72 text-sm font-medium">
      ...
      <section className="min-h-screen py-24 px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/20 border border-white/20">
          {architecture.map((item) => (
            <div key={item.title} className="bg-[#080808] p-10 md:p-12 aspect-square flex flex-col justify-between hover:bg-[#111] transition-colors group">
              <div className="w-8 h-8 border border-white/20 rounded-full group-hover:bg-white transition-colors" />
              <div>
                <h3 className="text-xl md:text-2xl font-serif italic mb-2">{item.title}</h3>
                <p className="text-[11px] md:text-xs uppercase tracking-widest text-white/75 leading-relaxed">{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <footer className="py-24 px-6 md:px-12 border-t border-white/10 flex flex-col md:flex-row justify-between items-start md:items-end gap-12">
        ...
        <div className="flex flex-col gap-4 text-right">
          <a href="https://join.aimindset.org/context" className="text-lg hover:italic transition-all">
            Brief
          </a>
          <a href="https://join.aimindset.org/waitlist" className="text-lg hover:italic transition-all">
            Blocks
          </a>
          <a href="https://t.me/ai_mind_set" className="text-lg hover:italic transition-all">
            Contact
          </a>
          <div className="mt-8 text-[10px] uppercase tracking-widest text-white/70">© 2026 Company Page System</div>
        </div>
      </footer>
    </div>
  );
